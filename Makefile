...
# ParÃ¢metros de execuÃ§Ã£o do comando MAKE
# OpÃ§Ãµes possÃ­veis para spe (sistema de proc eletronico): sei3, sei4, sei41, super
sistema=sei41
base=mysql
teste=
...
check-isalive: ## Target de apoio. Acessa os Sistemas e verifica se estao respondendo a tela de login
	...
	@for i in `seq 1 15`; do \
		...
		if $(CMD_CURL_LOGIN_ORG1); then \
				echo 'PÃ¡gina de login encontrada!' ; \
				break ; \
		fi; \
		sleep 5; \
	done; \
...
dist: cria_json_compatibilidade
	# ATENÃÃO: AO ADICIONAR UM NOVO ARQUIVO DE DEPLOY, VERIFICAR O MESMO EM VerificadorInstalacaoRN::verificarPosicionamentoScriptsConectado
	...
	@echo "ConstruÃ§Ã£o do pacote de distribuiÃ§Ã£o finalizada com sucesso"
...
clean:
	...
	@echo "Limpeza do diretÃ³rio de distribuiÃ§Ã£o do realizada com sucesso"
...
verify-config:
	@echo "Verificando configuraÃ§Ãµes do mÃ³dulo para instÃ¢ncia org1"
	$(CMD_COMPOSE_FUNC) exec org1-http php /opt/sei/scripts/mod-pen/verifica_instalacao_modulo_pen.php
	@echo "Verificando configuraÃ§Ãµes do mÃ³dulo para instÃ¢ncia org2"
...
tramitar-pendencias-silent:
	@echo 'Executando monitoramento de pendÃªncias do Org1 e Org2'
