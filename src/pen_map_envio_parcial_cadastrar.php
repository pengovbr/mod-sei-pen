...
<form id="frmGravarOrgaoExterno" name="frmGravarOrgaoExterno" method="post" action="<?php echo $strLinkValidacao ?>">
  ...
  <?php
    // A unidade so pode manter processo aberto se estiver na whitelist
    // EnvioMultiplosOrgaos da ConfiguracaoModPEN (PR #1231).
    $existeConfigMultiplosOrgao = (is_array($podeEnvioMultiplosOrgaos) && in_array($hdnIdUnidade, $podeEnvioMultiplosOrgaos));
    $displayNone = $existeConfigMultiplosOrgao ? '' : 'display: none;';
  ?>
  <div id="divSinMultiplosOrgaos" class="infraDivCheckbox" style="padding-top: 20px; <?php echo $displayNone; ?>">
    <input type="checkbox" id="sinMultiplosOrgaos" name="sinMultiplosOrgaos" class="infraCheckbox" tabindex="<?php echo PaginaSEI::getInstance()->getProxTabDados() ?>" <?php echo $existeConfigMultiplosOrgao && $sinMultiplosOrgaos === 'S' ? 'checked' : '' ?> />
