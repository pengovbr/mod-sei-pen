...
<form id="frmGravarOrgaoExterno" name="frmGravarOrgaoExterno" method="post" action="<?= $strLinkValidacao ?>">
    ...
    <div id="divRepositorioEstruturas" class="infraAreaDados" style="height: 4.5em;">
        <label id="lblRepositorioEstruturas" for="selRepositorioEstruturas" accesskey=""
            class="infraLabelObrigatorio">Repositório de Estruturas Organizacionais:</label>
        <select id="selRepositorioEstruturas" name="selRepositorioEstruturas"
            class="infraSelect" onchange="selecionarRepositorio();"
            <?= $disabilitarVisualizar ?> tabindex="<?= $objPaginaSEI->getProxTabDados() ?>">
            <?= $strItensSelRepositorioEstruturas ?>
        </select>

        <input type="hidden" id="txtRepositorioEstruturas" name="txtRepositorioEstruturas"
            <?= $disabilitarVisualizar ?> class="infraText" value="<?= $txtRepositorioEstruturas; ?>" />
    </div>
    ...
    <div id="divUnidadesUnidades" class="infraAreaDados" style="height: 4.5em;">
        ...
        <div class="alinhamentoBotaoImput">
            <input type="text" id="txtUnidade" name="txtUnidade" class="infraText infraReadOnly"
                ...
                value="<?= PaginaSEI::tratarHTML($strNomeUnidade); ?>"
                tabindex="<?= $objPaginaSEI->getProxTabDados() ?>" />
            <?php if ($_GET['acao'] != 'pen_map_envio_parcial_visualizar') { ?>
                <button id="btnIdUnidade" type="button" class="infraButton">Consultar</button>
                <img id="imgPesquisaAvancada" src="imagens/organograma.gif"
                    alt="Consultar organograma" title="Consultar organograma" class="infraImg" />
            <?php } ?>
        </div>
