...
class PenAtualizarSeiRN extends PenAtualizadorRN
{
   ...
   *
   * O insert em md_pen_anexo_documento gera um novo id (PK por sequencia, que
   * o InfraBD nao permite informar). Sem o reaponte, id_anexo_imutavel fica
   * orfao ou aponta para outro anexo que herdou o numero antigo.
   *
   * @param int $numIdAnexoOriginal id que o anexo tinha em `anexo`
   * @param int $numIdAnexoNovo     id que recebeu em md_pen_anexo_documento
   */
  ...
  /**
   * Garante que o indice de id_serie seja de fato UNICO (issue #1207).
   *
   * criarIndice() compara so as COLUNAS, ignorando o $bolUnique. Como o InnoDB
   * indexa coluna de FK sozinho, no MySQL ele apenas RENOMEIA esse indice e o
   * unico nunca e criado -- por isso as duplicidades se acumulam.
   */
  ...
  /**
   * Nome do indice que cobre exatamente a coluna id_serie, ou null.
   *
   * @param  bool|null $bolSomenteUnique true so indices unicos; null qualquer um
   * @return string|null
   */
  ...
  protected function instalarV4200_Beta()
  {
    ...
    $fnCadastrar = function ($strNome = '', $strHistoricoResumido = 'N', $strHistoricoCompleto = 'N', $strFecharAndamentosAbertos = 'N', $strLancarAndamentoFechado = 'N', $strPermiteProcessoFechado = 'N', $strIdTarefaModulo = '', $strSinConsultaProcessual = 'N') use ($objDTO, $objBD) {
      ...
      $fnCadastrar('A sincronização do processo @PROTOCOLO_FORMATADO@ foi cancelada pelo sistema de origem. Por favor, entre em contato com a equipe gestora desse sistema para entender o que motivou o encerramento da sincronia.', 'S', 'S', 'N', 'S', 'N', 'PEN_SINC_MULTIPLOS_ORGAOS_CANCELADO_AUTO');
  ...
  /**
   * Remove mapeamentos de envio duplicados, mantendo o mais recente de cada
   * id_serie, para que o indice unico da issue #1207 possa ser criado.
   *
   * Mantem a linha de maior id_mapeamento (a mais recente) e registra cada
   * remocao no log. Usa a camada BD porque o RN exige sessao de usuario,
   * inexistente em script de linha de comando.
   */
  ...
  private function listarAnexoDocumentosInternos($numLimite, $numUltimoIdAnexo = 0)
    ...
    /**
     * Normaliza o valor de dth_inclusao vindo de consultarSql() cru.
     *
     * A consulta e crua, entao a data chega na forma nativa de cada driver:
     *   - MySQL e PostgreSQL: string 'Y-m-d H:i:s'
     *   - Oracle:             string 'd/m/Y H:i:s'
     *   - SQL Server:         objeto DateTime
     *
     * Qualquer outra forma falha com mensagem explicita.
     *
     * @param  mixed $mixDataHora valor bruto devolvido pelo driver
     * @return string data no formato d/m/Y H:i:s esperado pelo InfraDTO
     * @throws InfraException
     */
