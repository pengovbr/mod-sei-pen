        ...
        try {
            ...
            if (len > 0) {
                if (confirm('Confirma a exclusão de ' + len + ' mapeamento(s) ?')) {
                    ...
                    form.attr('action', '<?php print $objSessaoSEI->assinarLink('controlador.php?acao=pen_map_envio_parcial_excluir&acao_origem=' . $_GET['acao_origem'] . '&acao_retorno=pen_map_envio_parcial_listar'); ?>');
                    form.submit();
                }
            } else {
