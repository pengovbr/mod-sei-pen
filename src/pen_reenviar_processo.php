...
try {
    ...
    switch ($_GET['acao']) {
        ...
        case 'pen_reenviar_processo':
            $strTitulo = 'Reenvio Externo de Processo';
            $arrComandos[] = '<button type="button" accesskey="R" onclick="enviarForm(event)" value="Reenviar" class="infraButton" style="width:8%;"><span class="infraTeclaAtalho">R</span>eenviar</button>';
            ...
            //Carregar dados do procedimento no primeiro acesso à página
            ...
            //
            $procedimentoAndamentoDTO = new ProcedimentoAndamentoDTO();
            $procedimentoAndamentoDTO->retNumIdEstruturaDestino();
            $procedimentoAndamentoDTO->retNumIdRepositorioDestino();
            $procedimentoAndamentoDTO->setDistinct(true);
            $procedimentoAndamentoDTO->setDblIdProcedimento($idProcedimento);

            $procedimentoAndamentoRN = new ProcedimentoAndamentoRN();
            $arrObjProcedimentoAndamentoDTO = $procedimentoAndamentoRN->listar($procedimentoAndamentoDTO);
            $objProcedimentoAndamentoDTO = $arrObjProcedimentoAndamentoDTO[0];

            $numIdRepositorio = $objProcedimentoAndamentoDTO->getNumIdRepositorioDestino();
            $numIdUnidadeDestino = $objProcedimentoAndamentoDTO->getNumIdEstruturaDestino();

            $objConecaoWebServerRN = new ProcessoEletronicoRN();
            $arrObjEstruturas = $objConecaoWebServerRN->consultarEstrutura($numIdRepositorio, $numIdUnidadeDestino, false);
            $strNomeUnidadeDestino = $arrObjEstruturas->getStrNome();
        ...
        if(isset($_POST['sbmExpedir'])) {
            ...
            $strTituloPagina = "Reenvio externo do processo $strProtocoloProcedimentoFormatado";
    ...
    throw new InfraException("Erro processando requisição de reenvio externo de processo", $e);
}
...
#txtProtocoloExibir {position:absolute;left:0%;top:38%;width:51%;}
...
#txtUnidade {left:0%;top:38%;width:60%;border:.1em solid #666;}
...
?>
    ...
    <div id="divRepositorioEstruturas" class="infraAreaDados" style="height: 4.5em;">
        ...
        <select id="selRepositorioEstruturas" name="selRepositorioEstruturas" class="infraSelect" onchange="selecionarRepositorio();" tabindex="<?= $objPaginaSEI->getProxTabDados() ?>" disabled>
