...
class PENIntegracao extends SeiIntegracao
{
  ...
  public function getNome() {
    return 'IntegraÁ„o Tramita GOV.BR';
  }
  ...
  {
    $this->validarExcluirDesativarHipoteseLegal($arrObjHipoteseLegalDTO, 'exclus„o');
  }

  public function desativarHipoteseLegal($arrObjHipoteseLegalDTO)
  {
    $this->validarExcluirDesativarHipoteseLegal($arrObjHipoteseLegalDTO, 'inativaÁ„o');
  }
    ...
    foreach ($arrObjHipoteseLegalAPI as $objHipoteseLegalAPI) {
      ...
      if (!is_null($objPenRelHipoteseLegalEnvioDTO) || !is_null($objPenRelHipoteseLegalRecebidoDTO)) {
        ...
        $excecao->lancarValidacao($this->getNome().": 
          A $strAcao da hipÛtese legal $nome n„o È permitida. 
          A referida hipÛtese legal est· relacionada a uma hipÛtese legal do Tramita."
        );
      }
    }
  }
  ...
  public function desativarTipoProcesso($arrObjTipoProcedimentoDTO)
  {
    ...
    $mensagem = "Prezado(a) usu·rio(a), vocÍ est· tentando desativar um Tipo de Processo que se encontra mapeado para o(s) relacionamento(s) "
          ."\"%s\". Para continuar com essa aÁ„o È necess·rio remover do(s) mapeamentos "
    ...
    $mensagem = 'Prezado(a) usu·rio(a), vocÍ est· tentando desativar o Tipo de Processo "%s" '
      . 'que se encontra mapeado para o Tipo de Processo Padr„o. '
      . 'Para continuar com essa aÁ„o È necess·rio alterar o Tipo de Processo Padr„o. '
      . 'O Tipo de Processo padr„o se encontra disponÌvel em: '
      . 'AdministraÁ„o -> Tramita GOV.BR -> Mapeamento de Tipos de Processo -> Relacionamento entre Unidades';
  ...
  public function excluirTipoProcesso($arrObjTipoProcedimentoDTO)
  {
    ...
    $mensagem = "Prezado(a) usu·rio(a), vocÍ est· tentando excluir um Tipo de Processo que se encontra mapeado para o(s) relacionamento(s) "
      ."\"%s\". Para continuar com essa aÁ„o È necess·rio remover do(s) mapeamentos "
      ."mencionados o Tipo de Processo: \"%s\".";
    ...
    $mensagem = 'Prezado(a) usu·rio(a), vocÍ est· tentando excluir o Tipo de Processo "%s" '
      . 'que se encontra mapeado para o Tipo de Processo Padr„o. '
      . 'Para continuar com essa aÁ„o È necess·rio alterar o Tipo de Processo Padr„o. '
      . 'O Tipo de Processo padr„o se encontra disponÌvel em: '
      . 'AdministraÁ„o -> Tramita GOV.BR -> Mapeamento de Tipos de Processo -> Relacionamento entre Unidades';
  ...
  /**
   * MÈtodo respons·vel de criar listagem de item para XML
   */
      ...
      case 'pen_listar_repositorios_estruturas_auto_completar':
        try {
          ...
          if (count($arrObjEstruturaDTO) > 0) {
            ...
            return '<itens><item grupo="vazio" id="0" descricao="RepositÛrio de Estruturas n„o Encontrado."></item></itens>';
          }
        }catch(Throwable $e){
          $mensagem = "Falha na obtenÁ„o dos RepositÛrios de Estruturas Organizacionais";
  ...
  public function assinarDocumento($arrObjDocumentoAPI = array()){
    ...
    foreach($arrObjDocumentoAPI as $objDocumentoAPI){
      ...
      //Se n„o tiver tr‚mite n„o h· o que se checar
      if(!is_null($objUltimoTramiteRecebidoDTO)){
        ...
        if(!is_null($arrObjComponentesDigitais)){
          foreach($arrObjComponentesDigitais as $objComponentesDigitais){
            if ($objComponentesDigitais->getDblIdDocumento() == $objDocumentoAPI->getIdDocumento()){
              ...
              $mensagem = "$nomeModulo: Prezado(a) usu·rio(a) esse documento j· foi tramitado externamente via Tramita GOV.BR. Por esse motivo, este documento n„o pode receber uma nova assinatura.";
