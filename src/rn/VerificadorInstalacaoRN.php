    ...
    */
    public function verificarArquivoConfiguracao()
    {
      ...
      if (file_exists($strArquivoConfiguracao) && is_readable($strArquivoConfiguracao)) {
          ...
          $strMensagem = "Arquivo de configuração do módulo de integração do SEI com o Barramento Tramita GOV.BR (mod-sei-pen) não pode ser localizado";
      ...
      if(!class_exists("ConfiguracaoModPEN")){
          $strMensagem = "Definição de configurações do módulo de integração do SEI com o Barramento Tramita GOV.BR (mod-sei-pen) não pode ser localizada";
      ...
      if(!array_key_exists("PEN", $arrStrChavesConfiguracao)){
          $strMensagem = "Grupo de parametrização 'Tramita GOV.BR' não pode ser localizado no arquivo de configuração do módulo de integração do SEI com o Barramento Tramita GOV.BR (mod-sei-pen)";
          $strDetalhes = "Verifique se o arquivo de configuração localizado em $strArquivoConfiguracao encontra-se íntegro.";
          throw new InfraException($strMensagem, null, $strDetalhes);
      }
      ...
      foreach ($arrStrParametrosExperados as $strChaveConfiguracao) {
        if(!array_key_exists($strChaveConfiguracao, $arrStrChavesConfiguracao)){
            $strMensagem = "Parâmetro 'PEN > $strChaveConfiguracao' não pode ser localizado no arquivo de configuração do módulo de integração do SEI com o Barramento Tramita GOV.BR (mod-sei-pen)";
            $strDetalhes = "Verifique se o arquivo de configuração localizado em $strArquivoConfiguracao encontra-se íntegro.";
            throw new InfraException($strMensagem, null, $strDetalhes);
        }
      }
    ...
    */
    public function verificarCertificadoDigital()
    {
      ...
      if(empty($strPublicKey)){
          throw new InfraException("Chave pública do certificado digital de autenticação no Barramento do Tramita GOV.BR não pode ser localizada em $strLocalizacaoCertificadoDigital. Erro detalhado: " . openssl_error_string());
      }
    ...
    public function verificarConexaoBarramentoPEN()
    {
      ...
      try{
        ...
        if(empty($strOutput) || $strOutput === false || empty($objXML) || $objXML === false){
            throw new Exception("Falha na validação do WSDL do webservice de integração com o Barramento de Serviços do Tramita GOV.BR localizado em $strEnderecoWSDL");
        }
    ...
    */
    public function verificarAcessoPendenciasTramitePEN()
    {
      ...
      try{
          ...
          throw new InfraException("Falha no acesso aos serviços de integração do Barramento de Serviços do Tramita GOV.BR: $e");
      }
    }
