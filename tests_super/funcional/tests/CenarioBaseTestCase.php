...
class CenarioBaseTestCase extends Selenium2TestCase
{
    ...
    protected function assinarDocumentoCertificadoDigital($siglaOrgao, $cargoAssinante, $loginSenha)
    {
        // Navegar para página de assinatura
        $this->paginaDocumento->navegarParaAssinarDocumento();
        sleep(2);
        ...
        $this->paginaAssinaturaDocumento->selecionarOrgaoAssinante($siglaOrgao);
        $this->paginaAssinaturaDocumento->selecionarCargoAssinante($cargoAssinante);
        $this->paginaAssinaturaDocumento->assinarComCertificadoDigital();
        $this->window('');
        sleep(2);
    }
    ...
    protected function assinarDocumentoTramitadoAnteriormente($remetente, $strProtocoloTeste) {
        ...
        // Verificando se a conexão com o banco de dados é oracle para fazer a conversão da data correta
        if ($conexaoSip->getAttribute(PDO::ATTR_DRIVER_NAME) == "oci") {
            $dataAnterior = "TO_DATE('". date('Y-m-d', strtotime('-1 day', strtotime(date('Y-m-d')))) . "00:00:01', 'YYYY-MM-DD HH24:MI:SS')";
        } else {
            $dataAnterior = "'".date('Y-m-d', strtotime('-1 day', strtotime(date('Y-m-d')))) . " 00:00:01'";
        }
        ...
        $idUsuarioSip = $usuario['ultimo_id'] + 1;
        $insertUsuarioSip = "insert into usuario (id_usuario, id_orgao, sigla, nome, sin_ativo, nome_registro_civil, sin_bloqueado) values (?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conexaoSip->prepare($insertUsuarioSip);
        $stmt->execute(array($idUsuarioSip, 0, 'testeuserdoc1', 'testeuserdoc1', 'S', 'testeuserdoc1', 'N'));

        // Insere a permissão básica para o usuário criado no sip
        $sql = "insert into permissao(id_perfil, id_sistema, id_usuario, id_unidade, id_tipo_permissao, dta_inicio, sin_subunidades) values(100000938, 100000100, ?, 110000000, 1, ". $dataAnterior .", 'N')";
        $stmt = $conexaoSip->prepare($sql);
        $stmt->execute(array($idUsuarioSip));
        ...
        $bancoOrgaoA->execute("insert into contato (id_contato, nome, sin_ativo, sta_natureza, sin_endereco_associado, id_contato_associado, id_tipo_contato, idx_contato, dth_cadastro, cpf) values (?, 'testeuserdoc1', 'S', 'J', 'N', ?, 6, 'TESTE 1 - USR', ".$dataAnterior.", '123456789')", array($idContato, $idContato));
        ...
        $bancoOrgaoA->execute($insertUsuario, array($idUsuario,'S','testeuserdoc1','TESTE 1 - USR', $idContato, 0, 'testeuserdoc1 teste 1 - usr', '0', 'N', 'TESTE 1 - USR'));

        // 1 - Acessar sistema do REMETENTE do processo
        $this->acessarSistema($remetente['URL'], $remetente['SIGLA_UNIDADE'], 'testeuserdoc1', 'testeuserdoc1');
        ...
        $this->assinarDocumento($remetente['ORGAO'], $remetente['CARGO_ASSINATURA'], 'testeuserdoc1');
    ...
    protected function assinarDocumentoCertificadoDigitalTramitadoAnteriormente($remetente, $strProtocoloTeste) {
        $this->acessarSistema($remetente['URL'], $remetente['SIGLA_UNIDADE'], 'testeuserdoc1', 'testeuserdoc1');
        ...
        $this->assertEquals(2, count($listaDocumentosProcessoPrincipal));
        $this->paginaProcesso->selecionarDocumento($listaDocumentosProcessoPrincipal[0]);
        $this->assinarDocumentoCertificadoDigital($remetente['ORGAO'], $remetente['CARGO_ASSINATURA'], 'testeuserdoc1');
        ...
        // Exclui o usuário criado no sip
        $stmt = $conexaoSip->prepare('SELECT ID_USUARIO FROM usuario WHERE sigla = ?');
        $usuario = $stmt->execute(array('testeuserdoc1'));

        $stmt = $conexaoSip->prepare('SELECT ID_USUARIO FROM usuario WHERE sigla = ?');
        $stmt->execute(array('testeuserdoc1'));
        $usuario = $stmt->fetchAll();
        
        $stmt = $conexaoSip->prepare("DELETE FROM permissao WHERE id_usuario = ?");
        $stmt->execute(array($usuario[0]['ID_USUARIO']));

        $stmt = $conexaoSip->prepare("DELETE FROM login WHERE id_usuario = ?");
        $stmt->execute(array($usuario[0]['ID_USUARIO']));

        $stmt = $conexaoSip->prepare("DELETE FROM usuario WHERE id_usuario = ?");
        $stmt->execute(array($usuario[0]['ID_USUARIO']));

        // // Conexão com o sei.
        $bancoOrgaoA = new DatabaseUtils(CONTEXTO_ORGAO_A);
        // Exclui o usuário criado no sei
        $usuario = $bancoOrgaoA->query("SELECT ID_USUARIO FROM usuario WHERE sigla = ?", array('testeuserdoc1'));
        $atividade = $bancoOrgaoA->query("SELECT ID_ATIVIDADE FROM atividade WHERE id_usuario_conclusao = ?", array($usuario[0]['ID_USUARIO']));

        $bancoOrgaoA->execute("DELETE FROM atributo_andamento WHERE id_atividade = ? ", array($atividade[0]['ID_ATIVIDADE']));
        $bancoOrgaoA->execute("DELETE FROM atividade WHERE id_usuario_conclusao = ? ", array($usuario[0]['ID_USUARIO']));
        $bancoOrgaoA->execute("DELETE FROM atividade WHERE id_usuario_origem = ? ", array($usuario[0]['ID_USUARIO']));
        $bancoOrgaoA->execute("DELETE FROM usuario WHERE id_usuario = ? ", array($usuario[0]['ID_USUARIO']));
        $bancoOrgaoA->execute("DELETE FROM contato WHERE nome = ?", array('testeuserdoc1'));
    }
}
