...
class PendenciasTramiteRN extends InfraRN {
        ...
        while (true) {
          ...
          if(isset($objPendenciaDTO)) {
              ...
              if($numIdTramiteRecebido != $objPendenciaDTO->getNumIdentificacaoTramite()) {
                ...
                // if(!isset($arrTramites[$numIdTramiteRecebido])){
                    // $arrTramites[$numIdTramiteRecebido] = true; 
  ...
  private function obterPendenciasTramite($parNumIdTramiteRecebido) 
  {
    ...
    //error_log("obterPendenciasTramite: curl " . $this->strEnderecoServicoPendencias . " --key " .  $this->strLocalizacaoCertificadoDigital . " --cert " .  $this->strLocalizacaoCertificadoDigital . " --pass 1234 --insecure", 0);
    ...
    //error_log("obterPendenciasTramite: curl: " . print_r(curl_getinfo($curl), true), 0);
    //error_log("obterPendenciasTramite: strResultadoJSON: " . print_r($strResultadoJSON, true), 0);
  ...
  private function enviarPendenciaFilaProcessamento($objPendencia)
  {
    ...
    switch ($objPendencia->getStrStatus()) {
        ...
        case ProcessoEletronicoRN::$STA_SITUACAO_TRAMITE_COMPONENTES_RECEBIDOS_DESTINATARIO:
            ...
            $numCont = 0;
            // Executa sempre + 1 além do configurado no parâmetro para executar 
            // a recusa
