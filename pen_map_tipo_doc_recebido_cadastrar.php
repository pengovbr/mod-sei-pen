...
try {
    ...
    //SessaoSEI::getInstance()->validarPermissao('pen_map_tipo_doc_recebido_cadastrar');
    ...
    switch ($_GET['acao']) {
        ...
        case 'pen_map_tipo_doc_recebido_visualizar':
            $arrComandos[] = '<button type="button" name="btnFechar" value="Fechar class="infraButton" onclick="location.href=\'' . $objPaginaSEI->formatarXHTML(SessaoSEI::getInstance()->assinarLink('controlador.php?acao=pen_map_tipo_doc_recebido_listar&acao_origem=' . $_GET['acao'])) . '\';">Fechar</button>';
    ...
    if(strtoupper($_SERVER['REQUEST_METHOD']) === 'POST') {
        ...
        header('Location: '.SessaoSEI::getInstance()->assinarLink('controlador.php?acao=pen_map_tipo_doc_recebido_listar&acao_origem='.$_GET['acao']));
        exit(0);
    }
