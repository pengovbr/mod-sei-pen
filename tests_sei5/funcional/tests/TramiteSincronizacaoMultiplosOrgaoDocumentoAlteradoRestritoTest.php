...
class TramiteSincronizacaoMultiplosOrgaoDocumentoAlteradoRestritoTest extends FixtureCenarioBaseTestCase
{
  ...
  public function test_alterar_origem_e_solicitar_sincronizacao(): void
  {
    ...
    $this->abrirProcesso(self::$protocoloTeste->getStrProtocoloFormatado());
    $this->alterarDocumentoParaRestrito();
    $this->sairSistema();
  ...
  public function test_validar_destino_sincronizado_restrito(): void
  {
    ...
    $listaDocumentos = $this->paginaProcesso->listarDocumentos();
    $this->assertNotEmpty($listaDocumentos);
    $this->assertDocumentoRestricao(
      $listaDocumentos[0],
      PaginaDocumento::STA_NIVEL_ACESSO_RESTRITO,
  ...
  protected function alterarDocumentoParaRestrito(): void
  {
    self::$documentoTeste['RESTRICAO'] = self::STA_NIVEL_ACESSO_RESTRITO;
    ...
    $this->paginaProcesso->selecionarDocumento($listaDocumentos[0]);
    ...
    $this->paginaDocumento->selecionarRestricao(
      PaginaDocumento::STA_NIVEL_ACESSO_RESTRITO,
      self::$remetente['HIPOTESE_RESTRICAO']
    );
    $this->paginaDocumento->salvarDocumento();
    sleep(2);
  }
  ...
  protected function assertDocumentoRestricao(string $nomeDocumentoArvore, int $nivel, ?string $hipoteseLegal = null): void
  {
    ...
    if ($hipoteseLegal !== null) {
      $this->assertEquals($hipoteseLegal, $this->paginaEditarProcesso->recuperarHipoteseLegal());
    }
  }

  protected function criarCenarioTramiteEnvioParcialTest(): void
  {
    self::$arrIdMapEnvioParcialOrgaoB = [];
    putenv("DATABASE_HOST=org2-database");
