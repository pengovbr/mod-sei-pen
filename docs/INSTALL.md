...
Esta seção descreve os passos obrigatórios para instalação do módulo de Integração do SEI com o Barramento de Serviços do PEN. Todos os itens descritos nesta seção são destinados à equipe de Tecnologia da Informação da instituição, responsáveis pela execução dos procedimentos técnicos de instalação e manutenção da infraestrutura do SEI.
...
### 4.1 Fazer backup dos bancos de dados do SEI, SIP e dos arquivos de configuração do sistema.

Todos os procedimentos de manutenção do sistema devem ser precedidos de backup completo de todo o sistema a fim de possibilitar a sua recuperação em caso de falha. A rotina de instalação descrita abaixo atualiza tanto o banco de dados, como os arquivos pré-instalados do módulo e, por isto, todas estas informações precisam ser resguardadas.

---

### 4.2. Baixar o arquivo de distribuição do mod-sei-pen

Necessário realizar o _download_ do pacote de distribuição do módulo mod-sei-pen para instalação ou atualização do sistema SEI. O pacote de distribuição consiste em um arquivo zip com a denominação mod-sei-pen-VERSAO.zip e sua última versão pode ser encontrada em https://github.com/spbgovbr/mod-sei-pen/releases.

---

### 4.3. Descompactar o pacote de instalação e atualizar os arquivos do sistema
...
### 4.4. Atualizar a base de dados do SIP com as tabelas do mod-sei-pen
...
### 4.4. Atualizar a base de dados do SEI com as tabelas do mod-sei-pen
...
#### 4.5. Reiniciar serviços de monitoramento de pendências de trâmite Gearman e Supervisor:

Necessário reiniciar os serviços opcionais de monitoramento de pendências de trâmite Gearman e Supervisord, caso estes dois tenham sido instalados e configurados.
...
### 6.1. Problema com validação de certificados HTTPS
...
### 6.2. Trâmites não realizados ou recibos não obtidos

Verificar se os serviços *gearman* e *supervisord* estão em execução, conforme orientado no manual de instalação, itens 3 e 4.

### 6.3. Erro na inicialização do Gearmand "(Address family not supported by protocol) -> libgearman-server/gearmand.cc:442"
...
### 6.4. Conversão do Certificado Digital do formato .P12 para o formato .PEM
...
### 6.5. Como obter o Fingerprint e Common Name do certificado digital
...
### 6.6. Configurar as permissões de segurança para os perfis e unidades que poderão realizar o trâmite externo de processos. 
